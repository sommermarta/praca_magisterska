...
Procesem gaussowskim nazywamy proces stochastyczny, którego rozkłady skończenie wielowymiarowe są normalne. Ktoś mógłby spytać: ,,W porządku, ale gdzie w klasyfikacji, w której jest skończenie wiele odpowiedzi, jest miejsce na rozkład normalny?" Otóż pomysł sam w sobie jest dość prosty. Mianowicie, zamiast bezpośrednio wnioskować ze zmiennych objaśniających $\textbf{X}$ o zmiennej odpowiedzi $\textbf{y}$, dodajemy krok pośredni tzw. zmienną ukrytą $\textbf{f}$. W uproszczony sposób predykcja wygląda zatem następująco: 
\begin{enumerate}
\item otrzymujemy na wejściu nową obserwację o danym wektorze cech $\textbf{x}_{\ast}$, 
\item w jakiś sposób wyliczamy dla niej liczbę rzeczywistą $f(x_{\ast})$,
\item  za pomocą pewnego przekształcenia prostej rzeczywistej na $r$ podzbiorów, wyznaczamy najlepszego $y_{\ast}$.
\end{enumerate}
 
Spróbujmy teraz zagłębić się w szczegóły. Interesuje nas obliczenie $\mathbb{P}(y_{\ast} | \textbf{X}, \textbf{y}, x_{\ast})$. Przy pomocy zmiennej ukrytej, możemy to rozpisać w następujący sposób:
$$
\mathbb{P}(y_{\ast} | \textbf{X}, \textbf{y}, x_{\ast}) = \int \mathbb{P}(y_{\ast} | f(x_{\ast}))\mathbb{P}(f(x_{\ast}) | \textbf{X}, \textbf{y}) df(x_{\ast}).
$$  
Musimy zatem wyestymować dwa prawdopodobieństwa pod powyższą całką, przy czym drugie z nich da się rozpisać w analogiczny sposób:
$$
\mathbb{P}(f(x_{\ast}) | \textbf{X}, \textbf{y}) = \int \mathbb{P}(f(x_{\ast}) | \mathbf{f})\mathbb{P}(\mathbf{f} | \textbf{X}, \textbf{y}) df.
$$ 
Szukamy więc trzech prawdopodobieństw: $\mathbb{P}(y_{\ast} | f(x_{\ast}))$, $\mathbb{P}(f(x_{\ast}) | \mathbf{f})$ oraz $\mathbb{P}(\mathbf{f} | \textbf{X}, \textbf{y})$. Przyjrzyjmy się im po kolei.
